...
public class NoticeController {
	...
	public String admon_notice_insert_ok(@ModelAttribute NoticeVO vo, HttpServletRequest request) throws Exception
	{
		...
		for(MultipartFile file:files)
		{
			...
			else
			{
				...
				if(f.exists())
				{
					String name=oname.substring(0,oname.lastIndexOf(".")); // . 미포함
					String ext=oname.substring(oname.lastIndexOf(".")); // . 포함
					int count=1;
					while(f.exists())
					{
